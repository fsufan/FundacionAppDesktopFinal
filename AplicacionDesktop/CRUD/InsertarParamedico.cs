...
{
    public partial class InsertarParamedico : Form
    {
        public InsertarParamedico()
        {
            InitializeComponent();
                            ...
                            if (DateTime.Compare(DateTime.Today, DateFechaNac.Value.Date) > 0)
                            {
                                if (DateTime.Compare(DateTime.Today, DateFechIngreso.Value.Date) <= 0)
                                {
                                    ...
                                    if (ComprobarFormatoEmail(txtEmail.Text) == true)
                                    {
                                        ...
                                        {
                                            DialogResult dialogResult = MessageBox.Show("Desea Agregar un nuevo Paramedico de nombre :" + txtNombre.Text + " " + txtApePaterno.Text, "Informacion", MessageBoxButtons.YesNo);
                                            if (dialogResult == DialogResult.Yes)
                                            {
                                                DialogResult resultado = MessageBox.Show("Se Ingresara el Paramedico ¿Desea Continuar?", "Atencion", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                                                switch (resultado)
                                                {
                                                        ...
                                                        if (auxDireccion.ingresarDireccion(direccion) > 0)
                                                        {
                                                            ...
                                                            if (auxInfoPersonal.IngresarInfoPersonal(infoPersonal) > 0)
                                                            {
                                                                ...
                                                                if (auxUsuario.ingresarUsuario(usuario) > 0)
                                                                {
                                                                    ...
                                                                    paramedico.Id_infopersonal = buscarinfo.Id_InfoPersonal;
                                                                    paramedico.Id_usuario = buscarUsuario.IdUsuario;

                                                                    if (auxParamedico.IngresarParamedico(paramedico) > 0)
                                                                    {
                                                                        ...
                                                                        MessageBox.Show("Paramedico Ingresado");
                                                                    }
                                                                    else
                                                                    {
                                                                        MessageBox.Show("Error al Ingresar un Paramedico");
                                                                    }
                                                                }
                                                                else
                                                                {
        ...
        {
            if ((!Regex.IsMatch(this.txtEmail.Text, @"^[A-Z0-9 a-z]*$")) && (txtEmail.Text != ""))
            {
                MessageBox.Show("Debe ingresar sólo caracteres validos");
                this.txtEmail.Focus();
                txtEmail.Text = "";
        ...
        {
            if ((!Regex.IsMatch(this.txtDescripcion.Text, @"^[A-Z0-9 a-z]*$")) && (txtDescripcion.Text != ""))
            {
                MessageBox.Show("Debe ingresar sólo caracteres validos");
